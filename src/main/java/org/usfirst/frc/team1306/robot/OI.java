 ...
 *     robot. It is also where commands can get joystick/trigger readings and
...
public class OI {
	...
	public OI() {
		...
		// Button pbuttonY = new JoystickButton(primaryController,
		// ControllerButton.Y.value);
		...
		// Button pbuttonStart = new JoystickButton(primaryController,
		// ControllerButton.START.value);
		// Button pbuttonBack = new JoystickButton(primaryController,
		// ControllerButton.BACK.value);
		 Button primaryDPadUp = new DPadPress(primaryController, DPadDirection.UP);
		// DPadPress primaryDPadRight = new DPadPress(primaryController,
		// DPadDirection.RIGHT);
		// Button primaryDPadLeft = new DPadPress(primaryController,
		// DPadDirection.LEFT);
		...
		// Button sbuttonA = new JoystickButton(secondaryController,
		// ControllerButton.A.value);
		Button sbuttonB = new JoystickButton(secondaryController, ControllerButton.B.value);
		// Button sbuttonX = new JoystickButton(secondaryController,
		// ControllerButton.X.value);
		...
		// Button sbuttonStart = new
		// JoystickButton(secondaryController,ControllerButton.START.value);
		// Button sbuttonBack = new JoystickButton(secondaryController,
		// ControllerButton.BACK.value);
		// Button secondaryDPadUp = new DPadPress(secondaryControlsler,
		// DPadDirection.UP);
		// Button secondaryDPadRight = new DPadPress(secondaryController,
		// DPadDirection.RIGHT);
		// Button secondaryDPadLeft = new DPadPress(secondaryController,
		// DPadDirection.LEFT);
		// Button secondaryDPadDown = new DPadPress(secondaryController,
		// DPadDirection.DOWN);
		...
		primaryDPadUp.whenPressed(new RetractArms());
		primaryDPadDown.whenPressed(new ActuateArms());
		/** @Secondary_Controls */

		// Testing code
		// sbuttonY.whenPressed(new TimedLift(ElevatorAction.LIFT, 2.1)); //TODO Remove
		// at 7R
		...
		// Normal Code
		// sbuttonA.whenPressed(new TimedLift(ElevatorAction.LIFT, 2.1)); //TODO Remove
		// at 7R
	...
	public enum Controller {
		P, S
	}; // Controller (primary or secondary)
	...
	public enum Side {
		L, R
	}; // Side (left or right) (for rumble)

	/**
	 * Returns the joystick value (from -1.0 to 1.0) for a specified controller's
	 * joystick's axis (uses deadband)
	 */
	public static double getJoyVal(Controller c, Joystick j, Axis a) {
		XboxController controller;
		Hand side;

		if (c.equals(Controller.P)) {
			controller = primaryController;
		} else {
			controller = secondaryController;
		}
		if (j.equals(Joystick.L)) {
			side = Hand.kLeft;
		} else {
			side = Hand.kRight;
		}
	...
	/**
	 * Applies deadband to joystick values to prevent false readings when joystick
	 * is idle.
	 */
